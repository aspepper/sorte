...
FROM	(
		...
		JOIN (SELECT MAX([DataConcurso]) AS [DataConcurso] FROM [dbo].[Megasenas] WHERE [DataConcurso]<'2020-08-08 00:00:00.0000000') MS ON MS.[DataConcurso] = MC.[DataConcurso]
...
SELECT max(Id) FROM [dbo].[MegasenaCidades];

DBCC CHECKIDENT ('Megasenas', RESEED, 3287);  
DBCC CHECKIDENT ('Sorteados', RESEED, 14704);  
DBCC CHECKIDENT ('MegasenaCidades', RESEED, 1795); 

DELETE FROM [dbo].[MegasenaCidades] WHERE [MegasenaId] IN (4288,4287);
DELETE FROM [dbo].[Sorteados] WHERE [MegasenaId] IN (4288,4287);
DELETE FROM [dbo].[Megasenas] WHERE [Id] IN (4288,4287);
